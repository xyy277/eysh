...
@Service
public class SmsTeleLogServiceImpl extends BaseServiceImpl<Sms_tele_log> implements SmsTeleLogService {
    ...
    public NutMap getData(int length, int start, int draw, List<DataTableOrder> orders, List<DataTableColumn> columns, Cnd cnd, String linkName, String beginDate, String endDate) {
        ...
        try {
            String sql = "select * from (SELECT * FROM sms_tele_log_ ";
            ...
            if (theSameMonth(beginDate, endDate)) {//判断是否为同一个月
                ...
                String tableName = "sms_tele_log_" + DateUtil.format(calendar.getTime(), "yyyyMM");
...
//        }
    ...
    private List<String> getMonthList(String date1, String date2)
        ...
        while (c4.after(c3)) {
            ...
            tempList.add("sms_tele_log_" + DateUtil.format(c3.getTime(), "yyyyMM"));
    ...
    private List<Record> getTableList() {
        return this.list(Sqls.create("SELECT table_name FROM information_schema.TABLES WHERE table_name like 'sms_tele_log_2%';"));
    }
