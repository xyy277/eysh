...
public class WxConfigController {
    ...
    @RequestMapping(value = {"/setMain/{id}"})
    @SJson
    @SLog(description = "Wx_config")
    @RequiresPermissions("wx.conf.account")
    public Object setMain(@PathVariable(required = false) String id, HttpServletRequest req) {
        try {

            Wx_config info = wxConfigService.fetch(Cnd.where("is_main" ,"=","1"));
            if (info!=null){
                info.setIs_main("0");
                wxConfigService.update(info);
            }
            info = wxConfigService.fetch(id);
            info.setIs_main("1");
            wxConfigService.update(info);
