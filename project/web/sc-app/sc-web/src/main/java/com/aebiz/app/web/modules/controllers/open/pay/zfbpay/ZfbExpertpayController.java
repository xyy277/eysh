 ...
 */
@Controller
@RequestMapping("/open/pay/zfbExpertpay")
public class ZfbExpertpayController {
    ...
    public void zfbPayOrder(@RequestParam("orderId") String orderId, HttpServletResponse httpResponse){
        try{
            //计算订单总金额
//            if(orderMain == null){
//                log.debug("订单不存在");
...
//            }
            ...
            order.setSubject("专家咨询");
            order.setBody("专家咨询");
            order.setUid("");
            order.setReturnurl("http://" + Globals.APP_DOMAIN + "/front/expert/paySuccess");
            order.setNotifyurl("http://" + Globals.APP_DOMAIN + "/open/pay/zfbExpertpay/payOrderBack");
