...
public class SysMenuController {
    ...
    public String edit(@PathVariable String id, HttpServletRequest req) {
        String dbType = sysMenuService.dao().getJdbcExpert().getDatabaseType();
        Sys_menu menu = sysMenuService.fetch(id);
        if (menu != null) {
            req.setAttribute("obj", menu);
            Sys_menu parentMenu = new Sys_menu();
            if (Strings.isBlank(menu.getParentId())) {
                if (DB.ORACLE.name().equals(dbType) || DB.DM.name().equals(dbType)) {
                    parentMenu = sysMenuService.fetch(Cnd.where(new Static(" id is null ")));
                } else {
                    parentMenu = sysMenuService.fetch(menu.getParentId());
                }
            } else {
                parentMenu = sysMenuService.fetch(menu.getParentId());
            }
            req.setAttribute("parentMenu", parentMenu);
