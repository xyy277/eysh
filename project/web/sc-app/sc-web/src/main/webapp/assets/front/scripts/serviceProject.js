var serviceProject = {
    init: function () {
        ...
        layui.use("laypage", function () {
            var a = layui.laypage;
            $.getJSON('/front/item/queryItems?'+queryCondition, {pageNo: 1}, function(res){ //从第6页开始请求。返回的json格式可以任意定义
                initDate(res.list);
                a.render({
                    elem: "m-page",
                    groups: 2, //连续显示分页数
                    ...
                    jump: function(e, first) { //触发分页后的回调
                        if (!first) { //一定要加此判断，否则初始时会无限刷新
                            $.getJSON('/front/item/queryItems?'+queryCondition, {pageNo: e.curr}, function(res){
                                //渲染
                                initDate(res.list);
                            });
                        }
